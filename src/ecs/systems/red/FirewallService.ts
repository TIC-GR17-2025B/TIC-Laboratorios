...
export class FirewallService {
  ...
  validarFirewall(
    ...
    const routers = this.conectividadService.obtenerRoutersDeRed(
      entidadOrigen,
      entidadDestino
    );
    ...
    const infoRouterOrigen =
      this.conectividadService.buscarRouterConDispositivo(entidadOrigen);
    const infoRouterDestino =
      this.conectividadService.buscarRouterConDispositivo(entidadDestino);
    ...
    if (
      infoRouterOrigen &&
      infoRouterDestino &&
      infoRouterOrigen.zonaId === infoRouterDestino.zonaId
    ) {
      // Encontrar la entidad del router
      const entidadRouter = this.encontrarEntidadRouter(
        infoRouterOrigen.router
      );
      if (entidadRouter) {
        return this.evaluarFirewallDeRouter(
          entidadRouter,
          infoRouterOrigen.router,
          entidadOrigen,
          entidadDestino,
    ...
    if (infoRouterOrigen) {
      const entidadRouterOrigen = this.encontrarEntidadRouter(
        infoRouterOrigen.router
      );
      if (entidadRouterOrigen) {
        ...
        if (!permitidoSaliente) {
          this.eventoService.emitirEventoBloqueado(
            nombreOrigen,
            nombreDestino,
            protocolo,
            "Router origen",
          ...
          this.ecsManager.emit(EventosRed.RED_FIREWALL_BLOQUEO, {
            descripcion: ` SE CAE EL MUNDO Tráfico bloqueado por firewall del router origen (${infoRouterOrigen.router}).`,
          });
          return false;
        }
      }
    ...
    if (
      ...
      const entidadRouterDestino = this.encontrarEntidadRouter(
    ...
    if (infoRouterDestino) {
      entidadRouterPermitido = this.encontrarEntidadRouter(
        infoRouterDestino.router
      );
    ...
    if (permitido) {
      this.eventoService.emitirEventoPermitido(
        nombreOrigen,
        nombreDestino,
        protocolo,
        entidadRouterPermitido ?? undefined
      );
    }
  ...
  private encontrarEntidadRouter(
  ...
  evaluarFirewallDeRouter(
    ...
    const redesRouter =
      this.conectividadService["obtenerRedesDelDispositivo"](entidadRouter);

    if (redesOrigen.length === 0 && redesDestino.length === 0) {
      return true;
    }

    const mismaRed = redesOrigen.some((redOrigen) =>
      redesDestino.includes(redOrigen)
    );
    ...
    const origenEnEsteRouter = redesOrigen.some((red) =>
      redesRouter.includes(red)
    );
    const destinoEnEsteRouter = redesDestino.some((red) =>
      redesRouter.includes(red)
    );
